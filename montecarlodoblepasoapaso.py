...
class MonteCarloDobleWindow(QMainWindow):
    ...
    def calcular(self):
        try:
            ...
            # --- Gráfico 3D ---
            # --- Gráfico 3D ---
            for ax_temp in self.figure.axes[:]:
                if ax_temp is not self.ax:
                    self.figure.delaxes(ax_temp)
            ...
            # 1) Volumen
            volumen = (bx - ax) * (by - ay)
            self.output.append("1) Cálculo del área de la región de integración:")
            self.output.append("   Fórmula:  A = (b - a) * (d - c)")
            self.output.append(f"   Sustitución: A = ({bx} - {ax}) * ({by} - {ay})")
            self.output.append(f"   Resultado: A = {volumen}")


            # 2) Generación de puntos
            self.output.append("2) Se generan N puntos aleatorios uniformes en la región:")
            self.output.append("   Región: [a,b] × [c,d]")
            self.output.append(f"   Valores: [{ax},{bx}] × [{ay},{by}] con N={n}")

            # 3) Evaluación de la función
            self.output.append("3) Evaluación de la función en cada punto:")
            self.output.append("   f(x_i,y_i) para i=1,...,N")

            # 4) Promedio
            promedio = sum(pz)/len(pz)
            self.output.append("4) Cálculo del promedio de los valores:")
            self.output.append("   Fórmula:  f̄ = (1/N) Σ f(x_i,y_i)")
            self.output.append(f"   Resultado: f̄ = {promedio}")

            # 5) Integral estimada
            self.output.append("5) Estimación de la integral:")
            self.output.append("   Fórmula:  Î = V * f̄")
            self.output.append(f"   Sustitución: Î = {volumen} * {promedio}")
            self.output.append(f"   Resultado: Î ≈ {integral}")

            # 6) Varianza y desvío estándar
            varianza = sum((v - promedio) ** 2 for v in pz) / (n - 1)
            self.output.append("6) Varianza y desvío:")
            self.output.append("   Fórmula:  s² = (1/(N-1)) Σ (f(x_i,y_i) - f̄)²")
            self.output.append(f"   Resultado: varianza ≈ {varianza}, desvío ≈ {desvio}")

            # 7) Error estándar
            self.output.append("7) Error estándar:")
            self.output.append("   Fórmula:  EE = V * (s / √N)")
            self.output.append(f"   Sustitución: EE = {volumen} * ({desvio}/√{n})")
            self.output.append(f"   Resultado: {error_estandar}")

            # 8) Intervalo de confianza
            z = norm.ppf(1 - (1-conf)/2)
            z_red = round(z, 3)
            self.output.append("8) Intervalo de confianza:")
            self.output.append("   Fórmula: IC = Î ± z * EE")
            self.output.append(f"   Sustitución: IC = {integral} ± {z_red} * {error_estandar}")
