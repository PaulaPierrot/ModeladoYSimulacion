...
def aproximar_pi_montecarlo(n, semilla):
    random.seed(semilla)
    ...
    pi_aprox = 4 * exitos / n
    error_rel = abs(math.pi - pi_aprox) / math.pi * 100

    return pi_aprox, error_rel, puntos_x, puntos_y, colores
...
class MonteCarloDobleWindow(QMainWindow):
    def __init__(self):
        ...
        self.pi_btn = QPushButton("Aproximar π con Monte Carlo")
        style_primary_button(self.pi_btn, accent="#22c55e")
        self.pi_btn.clicked.connect(self.calcular_pi)
        layout.addWidget(self.pi_btn)
    ...
    def calcular_pi(self):
        try:
            ...
            semilla = int(self.seed_input.text())

            pi_aprox, error_rel, px, py, colores = aproximar_pi_montecarlo(n, semilla)

            # --- Gráfico 2D ---
            self.figure.clear()
            ax2d = self.figure.add_subplot(111)
            ax2d.scatter(px, py, c=colores, s=5, alpha=0.6)
            ax2d.set_aspect("equal")
            ax2d.set_title(f"Aproximación de π con Monte Carlo (N={n})")
            ax2d.set_xlabel("x")
            ax2d.set_ylabel("y")
            ...
            # --- Salida ---
            self.output.clear()
            self.output.append("📊 Aproximación de π con Monte Carlo:\n")
            self.output.append(f"π aproximado ≈ {pi_aprox}")
            self.output.append(f"Error relativo porcentual = {error_rel:.6f}%")
            self.output.append(f"N = {n}, Semilla = {semilla}")
