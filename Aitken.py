...
_ALIASES = {
    'ln': 'log', 'sen': 'sin', 'tg': 'tan', 'ctg': 'cot', '√': 'sqrt',
}

def _normalize_tokens(s: str) -> str:
    s = (s or '').strip().replace('π', 'pi')
...
# =============== utilidades de despeje (reusadas de PF) 
...
def _find_power_terms(f, x):
    ...
    for t in sp.Add.make_args(F):
        ...
        if t.is_Pow and t.base == x and t.exp.is_integer and t.exp >= 2:
            terms.append((sp.Integer(1), int(t.exp), t)); continue
        if t.is_Mul:
            coef = sp.Integer(1); power = 0; ok = True
            for fac in t.args:
                if fac == x: power += 1
                elif fac.is_Pow and fac.base == x and fac.exp.is_integer and fac.exp > 0:
                    power += int(fac.exp)
                elif fac.has(x): ok = False; break
                else: coef *= fac
...
def _sup_gprime(gprime_num, a, b, grid_pts=101):
    ...
    vals = []
    for xv in xs:
        try: vals.append(abs(gprime_num(xv)))
...
def _extract_single_trig_zero_candidates(f, x, intervalo, x0):
    ...
    if A.func == sp.cos or (A.is_Mul and any(arg.func == sp.cos for arg in A.args)):
        ...
        if B.free_symbols == set() and B != 0:
            ...
            base = sp.simplify((sp.pi/2 - C)/B); paso = sp.simplify(sp.pi/B)
            cand += _const_candidates_from_arithmetic_progression(base, paso, intervalo, x0,
                "Despeje cos: x = pi/2 + k*pi (ajustada por B,C)")
    if A.func == sp.sin or (A.is_Mul and any(arg.func == sp.sin for arg in A.args)):
        ...
        if B.free_symbols == set() and B != 0:
            C = sp.simplify(arg - B*x)
            base = sp.simplify((-C)/B); paso = sp.simplify(sp.pi/B)
            cand += _const_candidates_from_arithmetic_progression(base, paso, intervalo, x0,
                "Despeje sin: x = k*pi (ajustada por B,C)")
    return cand

# ==================== 

def aiken_unificado(
    x0, tol=1e-6, maxiter=200,
    g_str=None, f_str=None,
    intervalo=None, grid_pts=101,
):
    """Devuelve: raiz, historia(list[dict]), g_plot (callable), g_expr (sympy|None), modo(str)"""
    x0_val = float(x0)
    ...
    if g_str and g_str.strip():
        ...
        x = sp.symbols('x')
        try:
            g_expr = sp.sympify(_normalize_user_expr_to_str(g_str), locals={'x': x, 'pi': sp.pi, 'E': sp.E})
        except Exception:
            g_expr = None
        modo = 'g_directa'
    else:
        ...
        x, f_expr, f_num = _build_functions(f_str)
        f_num_for_res = f_num
        candidatos = []
        candidatos += _extract_single_trig_zero_candidates(f_expr, x, intervalo, x0_val)
        a_lin, R = _split_linear_term(f_expr, x)
        if a_lin != 0:
            g1 = sp.simplify(-R / a_lin)
            candidatos.append(("Despeje lineal: a*x + R = 0 → x = -R/a", g1))
        for a_pow, n_pow, term in _find_power_terms(f_expr, x):
            Rpow = sp.simplify(f_expr - term)
        ...
        for desc, gexpr in candidatos:
            try:
                gp = sp.diff(gexpr, x)
                g_num_try  = sp.lambdify(x, gexpr, 'math')
                gp_num_try = sp.lambdify(x, gp,     'math')
                if intervalo is None:
                    q = abs(float(gp_num_try(x0_val))); ok = q < 1
                else:
                    a, b = intervalo; q = _sup_gprime(gp_num_try, a, b, grid_pts); ok = q < 1
            except Exception:
                ok = False
            if ok:
                elegido = (desc, gexpr, g_num_try); break
        if elegido is None:
            raise ValueError("Ningún despeje g(x) cumple |g'|<1 cerca de x0. Probá con otro x0 o método.")
        desc, g_expr, g_num = elegido
        modo = f"despeje: {desc}"
        # g callable para iteraciones
        try:
            g = crear_funcion_con_lhopital(str(g_expr))
        except Exception:
            g = lambda t: float(sp.N(g_num(t)))

    # --- derivada para chequeo de contracción (si se puede) ---
    gp_ok = False; gp_val = None
    try:
        if 'g_expr' in locals() and g_expr is not None:
            x = sp.symbols('x')
            gp_expr = sp.diff(g_expr, x)
            gp_val = float(sp.N(gp_expr.subs(x, x0_val)))
            gp_ok = True
    except Exception:
        gp_ok = False

    # --- iteración de Aitken/Steffensen ---
    historia = []
    xk = x0_val
    for k in range(1, maxiter + 1):
        try:
            ...
            raise ValueError("La evaluación de g(x) falló numéricamente durante la iteración.")
        denom = x2 - 2.0*x1 + xk
        xnext = x1 if abs(denom) < 1e-15 else (xk - (x1 - xk)**2 / denom)
        err = abs(xnext - xk)
        if f_num_for_res is not None:
            fval = float(f_num_for_res(xnext))
        elif 'g_expr' in locals() and g_expr is not None:
            x = sp.symbols('x'); f_disp_num = sp.lambdify(x, sp.simplify(g_expr - x), 'math')
            try: fval = float(f_disp_num(xnext))
            except Exception: fval = None
        else:
            fval = None
        historia.append({'k': k, 'xk': xk, 'x1': x1, 'x2': x2, 'xA': xnext, 'err': err, 'fval': fval})
        if err < tol:
            return xnext, historia, g, (g_expr if 'g_expr' in locals() else None), modo, (gp_ok, gp_val)
        xk = xnext
    return xk, historia, g, (g_expr if 'g_expr' in locals() else None), modo, (gp_ok, gp_val)
...
class AitkenWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Aitken/Steffensen – g(x) directa o despeje desde f(x)")
        ...
        self.g_input  = QLineEdit("cos(x)")
        self.f_input  = QLineEdit("cos(x) - x")
        self.p0_input = QLineEdit("0.5")
        ...
        self.btn = QPushButton("Calcular raíz (Aitken)")
        ...
        font = QFont("Consolas"); font.setStyleHint(QFont.StyleHint.Monospace); font.setPointSize(10)
    ...
    def insertar_texto(self, texto):
        if not self.active_input:
            return
        cur = self.active_input.cursorPosition(); s = self.active_input.text()
    ...
    def calcular(self):
        try:
            ...
            raiz, historia, g_plot, g_expr, modo, gp_info = aiken_unificado(
                x0=x0, tol=tol, g_str=g_str if g_str else None, f_str=f_str if f_str else None
            )

            # chequeo de contracción si tenemos g_expr
            gp_ok, gp_val = gp_info
            if gp_ok and abs(gp_val) >= 1:
                QMessageBox.warning(
                    self, "Atención",
                    f"La función g(x) podría no ser contractiva en x0={x0}.\n|g'(x0)| = {abs(gp_val):.4f} ≥ 1.\n"
                    "El método puede no converger."
                )

            # preparar f(x) para graficar si el usuario lo dio
            f_plot = None
            if f_str:
                try:
            ...
            for st in historia:
                k, xk, x1, x2, xA, err = st['k'], st['xk'], st['x1'], st['x2'], st['xA'], st['err']
                fval = st.get('fval', None)
                line = (
                    f"k={k:2d}:  x = {xk: .12f},  x1 = {x1: .12f},  x2 = {x2: .12f},  "
                    f"x_Aitken = {xA: .12f},  |Δ| = {err: .3e}"
                )
                if fval is not None:
                    line += f",  f(x) = {fval: .3e}"
